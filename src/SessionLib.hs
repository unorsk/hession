module SessionLib (generateMnemonic, generateMnemonicAndKeyPair) where
...
ptrToHex ptr = do
  bytes <- peekArray 16 ptr
  pure $ concatMap (printf "%02x") bytes
  where
    peekArray :: Int -> Ptr Word8 -> IO [Word8]
    peekArray n p = mapM (peekElemOff p) [0 .. n - 1]

generateMnemonicAndKeyPair :: IO (SessionKeyPair, Mnemonic)
generateMnemonicAndKeyPair = do
  (mnemonic, seedHex) <- generateMnemonic
  let seed = fromHex seedHex
  keyPair <- sessionGenerateKeyPair seed
  return (keyPair, mnemonic)

generateMnemonic :: IO (Mnemonic, String)
generateMnemonic = do
  ptr <- mallocBytes 16 :: IO (Ptr Word8)
  randombytesBuf ptr 16
  hexString <- ptrToHex ptr
  return (encodeMnemonic hexString, hexString)

--- >>> generateMnemonic
-- (["fudge","january","weekday","purged","general","dash","vivid","sugar","jeers","abyss","summon","acoustic"],"51fee986ba680bdae25d759f7644aa30")
  ...
  where
    ...
    chopped8ToMnemonics chopped8 =
      let x = (read $ "0x" <> chopped8) :: Int
          ...
          w1 = x `mod` n
          w2 = (x `div` n + w1) `mod` n
          w3 = ((x `div` n `div` n) + w2) `mod` n
       in [mnemonicSet !! w1, mnemonicSet !! w2, mnemonicSet !! w3]
...
decodeMnemonic mnemonic = concatMap mnemonicToHex mnemonic
  where
    -- mnemonicToHex word = case lookup word mnemonicSetIndexed of
    mnemonicToHex word = case findIndex (\m -> m == word) mnemonicSet of
      Just index -> printf "%02x" index
      Nothing -> error $ "Invalid mnemonic word: " ++ word

fromHex :: String -> [Word8]
fromHex hexString = map (\s -> ((read $ "0x" <> s) :: Word8)) $ chopBy 2 hexString

sessionGenerateKeyPair :: [Word8] -> IO SessionKeyPair
sessionGenerateKeyPair _seed = do
  let publicKey = "dummyPublicKey" -- Replace with actual key generation logic
  let privateKey = "dummyPrivateKey" -- Replace with actual key generation logic
  return $ SessionKeyPair (publicKey ++ privateKey)

generateKeypair :: Mnemonic -> IO SessionKeyPair
